...
public class DAOFile implements IDAO {
    ...
    public Person read(Person p) throws Exception {
        ...
        while (line != null) {
            ...
            if (data[1].equals(p.getNif())) {
                ...
                personToRead = new Person(data[0], data[1], data[2], date, data[4], data[5], photo);
                break;
            }
            line = br.readLine();
        }
    ...
    public ArrayList<Person> readAll() throws FileNotFoundException, IOException, ParseException, Exception {
        ...
        while (line != null) {
            ...
            people.add(new Person(data[0], data[1], data[2], date, data[4], data[5], photo));
            line = br.readLine();
        }
        br.close();
        return people;
    ...
    public void insert(Person p) throws IOException {
        ...
        if (p.getDateOfBirth() != null) {
            ...
            bw.write(p.getName() + "\t" + p.getNif() + "\t" + p.getEmail() + "\t" + dateAsString + "\t" + p.getPhoneNumber() + "\t" + p.getPostalCode() + "\t");
        } else {
            bw.write(p.getName() + "\t" + p.getNif() + "\t" + p.getEmail() + "\t" + "null" + "\t" + p.getPhoneNumber() + "\t" + p.getPostalCode() + "\t");
        }
        if (p.getPhoto() != null) {
